...
def _actions(token: str = "tok-1"):
    return approval_actions(token, windows=WINDOWS)
...
def test_every_control_carries_the_token_it_answers() -> None:
    ...
    assert all(a.context[SECRET_APPROVAL_KEY] == "tok-1" for a in actions)
...
def test_the_mattermost_codec_finds_the_approval_in_the_context() -> None:
    ...
    click = codec.parse_action(
        {
            "user_id": APPROVER,
            "context": {SECRET_APPROVAL_KEY: "tok-1", "value": ANSWER_ONCE},
        }
    )
    assert (click.secret_approval, click.value, click.user_id) == ("tok-1", ANSWER_ONCE, APPROVER)


def test_the_mattermost_codec_finds_a_pick_from_the_dropdown() -> None:
    codec = MattermostCallbackCodec()
    click = codec.parse_action(
        {"user_id": APPROVER, "context": {SECRET_APPROVAL_KEY: "tok-1", "selected_option": "grant:300"}}
    )
    assert (click.secret_approval, click.value) == ("tok-1", "grant:300")
...
def test_slack_round_trips_the_token_through_a_button_and_a_menu() -> None:
    ...
    assert decode_secret_approval(button) == "tok-1"
    ...
    assert decode_secret_approval(menu) == "tok-1"
...
def test_a_slack_click_that_is_not_an_approval_decodes_as_none() -> None:
    ...
    assert decode_secret_approval(button) == ""


# -- the dispatcher ------------------------------------------------------------


def _dispatcher(store, approvals: SecretApprovals | None) -> InteractionDispatcher:
...
async def test_without_a_broker_an_approval_click_is_nobody_s(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert dispatcher.resolve_secret_approval("tok-1", ANSWER_ONCE, APPROVER) is SecretApprovalOutcome.NOT_MINE
    finally:
        await store.close()
...
async def test_the_dispatcher_forwards_to_the_registry(tmp_path: Path) -> None:
    ...
    try:
        ...
        future = approvals.register(
            "tok-1", agent="assistant", secrets=("github-token",),
            approvers=frozenset({APPROVER}),
        )
        assert (
            dispatcher.resolve_secret_approval("tok-1", ANSWER_DENY, STRANGER) is SecretApprovalOutcome.NOT_ALLOWED
        )
        assert not future.done()
        assert dispatcher.resolve_secret_approval("tok-1", ANSWER_DENY, APPROVER) is SecretApprovalOutcome.RESOLVED
...
async def _receiver(port: int, store, approvals: SecretApprovals) -> InteractionsServer:
...
async def _click(port: int, token: str, value: str, user_id: str) -> dict:
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            json={"user_id": user_id, "context": {SECRET_APPROVAL_KEY: token, "value": value}},
        ) as response:
...
async def test_a_click_on_a_dead_request_retires_the_buttons(tmp_path: Path) -> None:
    store = SqliteSessionStore(tmp_path / "db.sqlite")
    server = await _receiver(8513, store, SecretApprovals())
    try:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                json={
                    "user_id": APPROVER,
                    "context": {SECRET_APPROVAL_KEY: "tok-1", "selected_option": "grant:300"},
                },
            ) as response:
                assert response.status == 200
        answer = await asyncio.wait_for(future, timeout=1)
